...
- Without `--target`, roadmapctl validates the whole frozen change-set before writing, orders parent/container changes before child tasks and existing README updates, writes all allowed changes, validates created/updated markdown, and runs the standard postcheck before reporting success.
- Batch apply accepts only allowlisted bootstrap changes (`.`, `.stem`, `.roadmapctl.toml`), canonical roadmap markdown creates (`TXXX-*.md`, `OXX-*/README.md`, or `OXX-*/TXXX-*.md`), and guarded `update` changes to existing `OXX-*/README.md` files generated by outcome append.
- Existing planned paths or stale README `previous_content` fail before writing via `RMC_MATERIALIZE_PLAN_CONFLICT`, and diagnostics identify the concrete blocking path.
- With `--target`, the target must match exactly one `changes[].path` entry; target apply writes only that selected canonical markdown create change, does not recompute numbering from the plan, and does not create sibling roadmap files.
