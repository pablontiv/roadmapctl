...
The controller is deterministic and read-model driven. Concrete status strings always come from the effective `.roadmapctl.toml` config plus the effective Rootline schema. Rules must not hardcode labels such as `Pending` or `Completed` except as built-in config defaults.
